...
I create a cube of atoms of width 6 for this experiment, Fig.~\ref{fig:first_simulation_ovito}

Choosing target temperature: 0.05 --- visualization shows that the atoms are pushed apart into infinity in small groups; 0.001 --- no vaporization.

Choosing Berendsen relaxation time (multiplied by \( \sqrt{m\sigma^2 / \epsilon} \)): for the whole simulation, \( \tau_T = 10 \) is required to keep the atoms from evaporating. Initially for $10$ time units I apply \( \tau_T = 1 \).
