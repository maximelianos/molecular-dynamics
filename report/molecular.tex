...
Equilibration with Berendsen thermostat is done for the first 100 fs with $\tau_T$ = 100 fs and target temperature 300 K = 27 C.

With correct force recomputation, even a huge time step $\Delta t=10$ fs causes total energy drift no more than 1 eV in 100 000 fs (without thermostat and heat deposition), we can safely use it for this simulation. Before the error was fixed, it drifted no matter which $\Delta t$ I chose. Also now I disable the thermostat after equilibration for fair energy measurements.
...
I simulated the Makay-923 and Makay-3871 Makay (TODO ref) clusters and generated Whisker-6950 using the \verb|make_whisker.py| script.
